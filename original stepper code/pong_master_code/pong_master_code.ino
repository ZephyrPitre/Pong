...
TimeNow2 = millis(); //The difference of the two times will increase with every iteration of the loop()
  
  if(TimeNow2 - TimeNow1 > 200) // when the difference is > 200, we enter
  {
  doSomething(); // we do something
  TimeNow1 = millis(); //then we refresh TimeNow1 to the current millis value. This "resets the timer"
  }
...
{    
  ...
  { // loop through all received character from the Slave (in this example, there is 1 character only)     
